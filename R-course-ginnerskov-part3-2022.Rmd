...
# Loading books from www.gutenberg.org (1/4)
...
# Loading books from www.gutenberg.org (2/4)
...
# Loading books from www.gutenberg.org (3/4)
...
---
# Loading books from www.gutenberg.org (4/4)
...
To facilitate our ability to interpret the results, it might be a good idea to visualize the tf-idf scores.  
```{r, echo = T, eval = F}
Socbooks.tfidf %>%
...
```{r, echo = T, eval = F}
library(widyr)
Socbooks.word.corr <- Socbooks.word.count %>% # reuse the object for 
  group_by(word) %>%
  filter(n >= 250) %>%
...
# Prepare the word correlations for viz
```{r, echo = T, eval = F}
library(igraph)
set.seed(1234)
...
---
# Viz the word correlation graph (1/2)
```{r, echo = T, eval = F}
library(ggraph)
Socbooks.word.corr.graph %>% 
  ggraph(layout = "fr") +
...
---
# Viz the wordcorrelation graph (2/2)
```{r}
library(ggraph)
Socbooks.word.corr.graph %>% 
  ggraph(layout = "fr") +
...
ggplot(Socbooks.senti.comp, aes(index, sentiment, fill = author)) +
