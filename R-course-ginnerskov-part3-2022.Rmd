...
In this analysis, we are goind to work with tidytext, which is a package set out to make text analysis friendly for the tidyverse universe. This is great since we can then add on other tidyverse packages like ggplot2.
```{r, echo = T, eval = F}
...
To get some insights into how the most distinct words in the corpus are interrelated, we can calculate word correlations. 
```{r, echo = T, eval = F}
...
You might also be interested in modeling the most prevalent topics that run through your corpus. Step one is to create a document-term matrix (dtm).
```{r, echo = T, eval = F}
...
---
# Run a topic model
Now we are ready for topic modeling. This time we will go for the Latent Dirichlet Allocation (LDA), a classic and  popular topic model. 
```{r, echo = T, eval = F}
library(topicmodels)
Socbooks.lda <- LDA(Socbooks.dtm, # function for generating a LDA
                    k = 4, # in toopic modeling, you have to slect the number of topics (k); go for 4
...
Let us visualize how the topics and books are related.
```{r, echo = T, eval = F}
Socbooks.lda.gamma %>%
